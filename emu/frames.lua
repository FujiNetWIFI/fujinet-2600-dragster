...
-- picture.
--
-- Dragster measures 262 lines, which is what a textbook NTSC frame is -- and
-- unlike Combat, which has no overscan and runs to 259, that is not a
-- coincidence worth leaning on. The gate asserts a CONSTANT learned from the
-- run rather than a number from a specification, and reports it, because the
-- number changing between stock and the split build is itself the finding.
--
-- The two seams are the thing under test. Seam B lands two instructions after
-- $F221 arms the overscan timer and seam A lands inside the vertical blank
-- timer armed at $F2C5, so both switches are paid for out of budgets that are
-- already running -- and if either were not, this is where a frame would grow.
--
--   ./run.sh dragster frames
