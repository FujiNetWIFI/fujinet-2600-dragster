/* Atari 2600 Dragster netplay relay -- C implementation.
 *
 * Port 9601 -- Combat's room is 9600.  A transliteration of
 * ../../dragster_relay_server.py, which stays the canonical reference.  Function names and their order in this file match the
 ...
 * WIRE.  The 2600 has no buffer to reassemble a split frame in, so the console
 * reads `(avail >> 3) << 3` bytes and a partial frame is simply not read this
 * time.  Session frames -- HELLO, START, LOBBY -- are NOT padded; the boot
 * bank reads them one at a time and knows each one's length.
 *
...
/* Every frame a console can receive DURING PLAY is this long on the wire. */
#define GAME_FRAME              8
#define GAME_PAYLOAD            (GAME_FRAME - 2)

/* Offsets inside an INPUT payload: tick(1) window(3) crc(1) check(1).  The
 * checksum moved from byte 2 to byte 4 when every record started carrying a
 * WINDOW of three ticks of input instead of one byte.  The relay reads only
 * the tick and the checksum. */
#define IR_TICK                 0
#define IR_CRC                  4
/* log_crc's guard.  Dragster wants the check byte present too; Video Olympics'
 * is one byte laxer.  Transcribed from each game's own Python, not unified. */
#define CRC_MIN_PAYLOAD         6

#define PROTO                   2   /* 2 put the television standard between
                                     * the version byte and the name, which is
...
/* TWO games, not Combat's twenty-seven.  Game 1 is a straight drag race and
 * game 2 adds the lane drift you have to steer against; the only difference in
 * the ROM is the block gated at $F466 on bit 0 of the variation cell. */
#define VARIATION_MIN           0
#define VARIATION_MAX           1
...
static int    g_port       = 9601;
static int    g_delay      = DEFAULT_DELAY;
static int    g_variation;
...
static void on_hello(int ci, const uint8_t *payload, size_t plen)
{
     ...
     * bits are ignored entirely.
     *
     * COMBAT REFUSED SECAM FOR A REASON THAT IS NOT DRAGSTER'S.  Its two tanks
     * are the same shape and are told apart only by colour, so on a SECAM set
     * two players could be looking at identical sprites.  Dragster tells its
     * cars apart by which lane they are in, and a SECAM set would show that
     * perfectly well.
     *
     * It is refused anyway, because no SECAM image exists to pair: the colour
     * table at $F6CA is hue-luminance and nobody has authored a SECAM one or
     * looked at the result on a real set.  build.sh refuses to produce one for
     * the same reason, and the two refusals have to agree or a console would
     * be turned away at the door for declaring something it could not have
     * been built to declare. */
    c->tv = payload[1];
    if (c->tv == TV_SECAM) {
        LOG("%s: refusing a SECAM console -- no SECAM Dragster image "
            "exists to pair it with", c->ipstr);
        drop(ci);
        return;
    }
...
static void try_pair(int ci)
{
    ...
    seed = next_seed();
    /* The console that was already waiting is the host and drives the left
     * lane -- the same convention as the Intellivision family, where the
     * waiting player takes role 0. */
    host  = other;
    guest = ci;
...
{
    fprintf(f,
"usage: dragster-relay [options]\n"
"  --host ADDR              bind address (default 0.0.0.0)\n"
"  --port N                 listen port (default 9601)\n"
"  --delay N                input delay in ticks (default %d; measured at\n"
"                           3 frames a tick)\n"
"  --variation N            Dragster game variation the match starts on,\n"
"                           %d or %d\n"
"  --lobby-url URL          e.g. http://lobby.fujinet.online:8080; off by\n"
"                           default.  http:// only -- this build has no TLS\n"
"  --lobby-appkey N         FujiNet-registry appkey id (default 25)\n"
...
int main(int argc, char **argv)
{
    ...
    const char *lobby_srvurl = "TCP://fujinet.online:9601/";
    const char *lobby_cliurl =
        "TNFS://apps.irata.online/Atari2600/Games/Dragster.bin";
    const char *region       = "us";
    const char *game_name    = "Dragster";
    const char *server_name  = "Dragster Netplay";
    int    lobby_appkey    = 25;
...
#undef NEEDVAL
    ...
    if (g_variation < VARIATION_MIN || g_variation > VARIATION_MAX) {
        fprintf(stderr, "dragster_relay: variation must be %d or %d\n",
                VARIATION_MIN, VARIATION_MAX);
        return 1;
    }
    if (g_port < 1 || g_port > 65535) {
        fprintf(stderr, "dragster_relay: port must be 1..65535\n");
        return 2;
    }
    if (g_fixed_seed < 0 || g_fixed_seed > 0xFFFF) {
        fprintf(stderr, "dragster_relay: --fixed-seed must be 1..65535\n");
        return 2;
    }
    ...
    if (inet_pton(AF_INET, g_host, &sa.sin_addr) != 1) {
        ...
        if (getaddrinfo(g_host, NULL, &hints, &res) != 0 || !res) {
            fprintf(stderr, "dragster_relay: cannot resolve --host %s\n", g_host);
            return 1;
        }
