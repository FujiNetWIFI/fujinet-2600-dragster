#!/usr/bin/env python3
"""dragster_relay_server.py -- the relay for networked Atari 2600 Dragster.

Port 9601 -- Combat's room is 9600. A fork of the Intellivision family's
relay, whose single most
...
split frame in -- the 128 bytes of console RAM are Dragster's, and the reply
...
Usage: dragster_relay_server.py [--port 9601] [--delay 2] [--lobby-url ...]
"""
...
PROTO = 2                   # 2 put the television standard between the version
                            # byte and the name, which is a payload SHAPE change
...
class Server:
    ...
    def handle(self, c, ftype, payload):
        if ftype in RELAY_TYPES:
            ...
            if ftype == T_INPUT and len(payload) >= 6:
    ...
    def on_hello(self, c, payload):
        ...
        #
        # COMBAT REFUSED SECAM FOR A REASON THAT IS NOT DRAGSTER'S. Its two
        # tanks are the same shape and are told apart only by colour, so on a
        # SECAM set two players could be looking at identical sprites. Dragster
        # tells its cars apart by which lane they are in, and a SECAM set would
        # show that perfectly well.
        #
        # It is refused anyway, because no SECAM image exists to pair: the
        # colour table at $F6CA is hue-luminance and nobody has authored a SECAM
        # one or looked at the result on a real set. build.sh refuses to produce
        # one for the same reason, and the two refusals have to agree or a
        # console would be turned away at the door for declaring something it
        # could not have been built to declare.
        c.tv = payload[1]
        if c.tv == TV_SECAM:
            log("%s: refusing a SECAM console -- no SECAM Dragster image "
                "exists to pair it with", c.addr[0])
            self.drop(c)
            return
    ...
    def log_crc(self, c, payload):
        ...
        """
        # payload is tick(1) window(3) crc(1) check(1): the checksum moved
        # from byte 2 to byte 4 when every record started carrying a WINDOW of
        # three ticks of input instead of one byte. See the note in cbnet.inc --
...
def main():
    ...
    ap.add_argument("--variation", type=int, default=0,
                    help="Dragster game variation the match starts on, 0 or 1")
    ap.add_argument("--lobby-url", default="",
                    help="e.g. https://lobby.fujinet.online/server; off by default")
    ap.add_argument("--lobby-appkey", type=int, default=25)
    ap.add_argument("--lobby-serverurl", default="TCP://fujinet.online:9601/")
    ap.add_argument("--lobby-client-url",
                    default="TNFS://apps.irata.online/Atari2600/Games/Dragster.bin")
    ap.add_argument("--game-name", default="Dragster")
    ap.add_argument("--server-name", default="Dragster Netplay")
    ap.add_argument("--region", default="us")
    args = ap.parse_args()
    # TWO games, not Combat's twenty-seven. Game 1 is a straight drag race and
    # game 2 adds the lane drift you have to steer against; the only difference
    # in the ROM is the block gated at $F466 on bit 0 of the variation cell.
    if not 0 <= args.variation <= 1:
        sys.exit("dragster_relay: variation must be 0 or 1")
    Server(args).run()
