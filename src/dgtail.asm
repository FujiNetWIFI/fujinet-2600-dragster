; dgtail.asm -- the fixed half: the trampoline, the shared transport, and the
; cold stub.
;
...
DGBANK  EQU     BANKBOOT
DGHASTXT EQU    1               ; the tail carries the text primitives for now;
                                ;   if it ever runs short they move into bank 0,
                                ;   which is the only bank that draws text

; DGGOTO is not a label: dgdefs.inc gives it a fixed address and this ORG is
...
; ---------------------------------------------------------------------------
; DGGOTO -- select bank A and enter it at $1000.
;
...
; Dragster wants SP = $FF at the top of a frame anyway -- its own clear loop
; does exactly this, a TXS per iteration ending at $FF -- so here the reset is
; both free and correct.
;
...
;
; It forces a cold entry by zeroing DGENT. This console does not clear its RAM
...
;
; Note that Dragster's RESET switch is NOT this path: the console's RESET line
; goes to the 6507 and restarts it here, but the game's own RESET handling at
; $F2E3 is an ordinary read of SWCHB during a frame, and restages the race.
DGCOLD: sei
        cld
        ldx     #$FF
        txs
