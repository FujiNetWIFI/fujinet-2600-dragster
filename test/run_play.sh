...
for n in 1 2; do
    ...
    cp build/dragster.bin "build/dragster$n.bin"
done
...
setsid python3 server/dragster_relay_server.py --host 127.0.0.1 \
    --port "$RELAY_PORT" --delay 2 --variation "${VARIATION:-0}" \
...
launch() {   # launch <n> <boip-port>
    ...
    ( cd "$MAME" && setsid env FUJINET_TCP="127.0.0.1:$port" \
        ...
        ./mame a2600 -window -nomaximize -resolution 640x480 \
            -skip_gameinfo \
            -cartslot fujinet -cart "$HERE/build/dragster$n.bin" \
