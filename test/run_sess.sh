...
PLAYER=PLAYER1 ENDPOINT="TCP://127.0.0.1:$RELAY_PORT/" ./build.sh dragster >/dev/null 2>&1
cp build/dragster.bin build/dragster1.bin
...
setsid python3 server/dragster_relay_server.py --host 127.0.0.1 --port "$RELAY_PORT" \
    < /dev/null > build/rig/relay.log 2>&1 &
RELAY_PID=$!
sleep 1

SECS="$SECS" FUJINET_TCP="127.0.0.1:$BOIP" ./run.sh dragster1 sess \
