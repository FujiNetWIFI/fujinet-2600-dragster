...
#
# SERVER=py (default) runs server/dragster_relay_server.py, which stays the
...
relay_server() {
    case "${SERVER:-py}" in
    py)
        exec python3 server/dragster_relay_server.py "$@"
        ;;
    c)
        if [ ! -x server/c/dragster-relay ]; then
            echo "SERVER=c: server/c/dragster-relay is not built -- run" >&2
            echo "  make -C server/c" >&2
            exit 1
        fi
        exec server/c/dragster-relay "$@"
        ;;
    *)
