...
findings were inside tables. That is not enough here. Dragster's sim clock lives
...
SLOTS = [("build/dgboot.lst", 0x1000),
         ("build/dggame.lst", 0x1000),
         ("build/dgkern.lst", 0x1000),
         ("build/dgtail.lst", 0x1800)]

FINDING = re.compile(
    r"^checkrom: (?P<img>\S+): bank (?P<bank>\d+) \$(?P<addr>[0-9A-Fa-f]{4}): (?P<what>.*)$")

# `(1)  312/1027 : E0 DA        CPX  #DGZPLO`  -- depth prefix optional
