#!/usr/bin/env python3
"""dragster_client_sim.py -- two simulated consoles against a fresh relay.
...
Usage: dragster_client_sim.py [--port 9639]
"""
...
def relay_argv():
    ...
    server/dragster_relay_server.py stays the canonical reference
    implementation.  SERVER=c runs the C port in server/c/, which
    tools/server_diff.py holds to byte-for-byte equality with it on the wire
    and in the log.
    """
    impl = os.environ.get("SERVER", "py")
    if impl == "py":
        return [sys.executable, os.path.join(HERE, "server/dragster_relay_server.py")]
    if impl == "c":
        binary = os.path.join(HERE, "server/c/dragster-relay")
...
class Sim:
    def __init__(self, port, name=None):
        ...
        self.name = name
        if name:
            self.s.sendall(frame(T_HELLO, bytes([2, TV_NTSC]) + name.encode()))
...
def main():
    ...
    try:
        ...
        check(pa[4] == pb[4] == 1, "both get the variation the server was told")
        ...
        for tick in range(50):
            ...
            if ra is None or rb is None:
                ok = False
                break
            # Each console must receive the OTHER's record, padded to the fixed
            # eight bytes on the wire -- six of payload after len and type.
            if len(ra[1]) != 6 or len(rb[1]) != 6:
                sized = False
            if rb[1][:6] != rec(tick, 0xF0 | (tick & 7)):
                ok = False
            if ra[1][:6] != rec(tick, 0x0F ^ (tick & 7)):
                ok = False
        check(ok, "50 INPUT frames each way, payloads verbatim")
        check(sized, "every relayed INPUT is padded to 8 bytes on the wire")
        ...
        # The checksum is the FIFTH payload byte now, behind the three-tick
        # window -- the one thing about the record layout the relay knows.
        a.send(T_INPUT, bytes([200, 0xFF, 0xFF, 0xFF, 0x11, 0]))
        b.send(T_INPUT, bytes([200, 0xFF, 0xFF, 0xFF, 0x11, 0]))
        a.send(T_INPUT, bytes([201, 0xFF, 0xFF, 0xFF, 0x22, 0]))
        b.send(T_INPUT, bytes([201, 0xFF, 0xFF, 0xFF, 0x33, 0]))
        time.sleep(0.3)
        ...
        check(pl is not None and len(pl[1]) == 6,
              "PEER_LEFT is padded to 8 bytes on the wire like every game frame")
        a.close()
        ...
        f = Sim(args.port)
        f.s.sendall(frame(T_HELLO, bytes([2, TV_SECAM]) + b"FRANCE"))
        ...
        g = Sim(args.port)
        g.s.sendall(frame(T_HELLO, bytes([2, TV_PAL]) + b"EUROPE"))
        ...
        e = Sim(args.port)
        e.s.sendall(frame(T_HELLO, bytes([2, TV_NTSC]) + b"x"))
        ...
        f = Sim(args.port)
        for byte in frame(T_HELLO, bytes([2, TV_NTSC]) + b"DRIBBLE"):
