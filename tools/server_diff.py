...
server/dragster_relay_server.py is canonical.  server/c/dragster-relay is a C port
...
RELAY_C = os.path.join(REPO, "server", "c", "dragster-relay")
RELAY_PY = os.path.join(REPO, "server", "dragster_relay_server.py")
...
def inrec(tick, win=(0xFF, 0xFF, 0xFF), crc=0x00):
    """One INPUT payload: tick(1) window(3) crc(1) check(1)."""
    b = bytes([tick, win[0], win[1], win[2], crc])
    return b + bytes([b[0] ^ b[1] ^ b[2] ^ b[3] ^ b[4] ^ 0xA5])
