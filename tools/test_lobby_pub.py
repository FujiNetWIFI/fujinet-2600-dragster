...
def main():
    ...
    relay = subprocess.Popen(
        ...
        ["--host", "127.0.0.1", "--port", "9641",
         ...
         "--game-name", "Dragster", "--server-name", "Dragster Netplay"],
    ...
    try:
        ...
        check(p.get("game") == "Dragster", "it publishes the game name")
        ...
        s.sendall(bytes([3 + len(name), 0x01, 2, 0]) + name)  # len counts type+payload
        for _ in range(60):
            if len(posts) > before:
                break
            time.sleep(0.05)
